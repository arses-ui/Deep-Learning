...
def validate(model, device, val_loader, criterion):
    ...
    with torch.no_grad():
        for data, target in val_loader:
            ...
            temp_margin = torch.tensor([pred - output.max(2)[-1]])
            margin = torch.cat([margin, temp_margin], dim=1)
    ...
    # NOT sure
    margin = torch.sort(margin)
    percentile_margin = margin[len(margin)*0.05]
    # *********** Your code ends here *************
...
def load_cifar10_data(split, datadir):
    ...
    # Not sure if supposed to apply this
